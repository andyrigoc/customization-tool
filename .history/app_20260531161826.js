...
function updateTextSizeLabels() {
  const widthCm = (textLayer.offsetWidth / 8.9).toFixed(2);
  const heightCm = (textLayer.offsetHeight / 8.9).toFixed(2);
...
function updateLogoSizeLabels() {
  const widthCm = (designLayer.offsetWidth / 8.9).toFixed(2);
  const heightCm = (designLayer.offsetHeight / 8.9).toFixed(2);
  const label = `${widthCm}cm x ${heightCm}cm`;
