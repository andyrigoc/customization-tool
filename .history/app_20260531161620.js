...
function updateTextSizeLabels() {
  const widthCm = (textLayer.offsetWidth / 7.1).toFixed(2);
  const heightCm = (textLayer.offsetHeight / 7.1).toFixed(2);
...
function updateLogoSizeLabels() {
  const widthCm = (designLayer.offsetWidth / 7.1).toFixed(2);
  const heightCm = (designLayer.offsetHeight / 7.1).toFixed(2);
  const label = `${widthCm}cm x ${heightCm}cm`;
